...
shipped product" from default model output. Apply every must-fix rule during
agent critique before presenting a result. Guidance rules still require
review, but are not claimed to be enforced automatically by the host.

> Adapted from [refero_skill](https://github.com/referodesign/refero_skill)
> (MIT) and revised for the host-independent MonkeyDesign runtime.

## The seven cardinal sins

These patterns are must-fix critique failures:

1. **Default Tailwind indigo as accent** — exactly `#6366f1`, `#4f46e5`,
   `#4338ca`, `#3730a3`, `#8b5cf6`, `#7c3aed`, `#a855f7`. Resolve the
   selected design system explicitly and use its accent token. Indigo is
   the textbook AI tell; flag hard-coded uses during critique.
...
- **Missing stable section identifiers** — interactive review and later
  edits need durable targets; use host-neutral `id` or `data-section-id`
  attributes rather than runtime-specific markers.
