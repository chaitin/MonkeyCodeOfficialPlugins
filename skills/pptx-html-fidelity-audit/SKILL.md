...
description: Audit a PPTX export against its source HTML deck, identify layout/content drift, repair systemic causes, and verify the result.
---

# PPTX ↔ HTML Fidelity Audit

Migrated for MonkeyCode from the OpenDesign resource under its Apache-2.0 license; original attribution and technical guidance are retained. See `LICENSE`.
...
First inspect the export mode. When `<deck>.pptx.fidelity.json` exists with `mode: high-fidelity-image`, verify that its slide count matches the HTML and PPTX, every slide contains one `full-slide-render` picture spanning the complete canvas, the manifest has one SHA-256 entry per slide, its `pptx_sha256` matches the delivered PPTX, and `verify_layout.py` reports no canvas violations. The footer-rail rule does not apply inside a whole-slide render because its editable source is the HTML; disclose that the PPTX is non-editable. Still visually inspect the rendered HTML pages for clipping before export. Do not pretend image-only slides have editable text-run or font fidelity to audit.

For editable/native-shape exports, the audit is five steps. Don't skip any of them — the discipline only works if the audit produces a real list of issues to drive the re-export. A fix-without-audit pass tends to leave half the issues alive.
...
- Asserts `top + height ≤ CONTENT_MAX_Y` for content shapes (only semantically named footer/page-number shapes are allowed below the rail; position alone never grants an exemption).
